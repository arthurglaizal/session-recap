Crée une commande Claude Code réutilisable dans ce projet appelée **Session Recap**.

Objectif : installer une commande `/session-recap` qui génère un récapitulatif lisible de la session de travail en cours (résumé, déroulé chronologique, état du projet, prochaine étape), fondé uniquement sur ce qui s'est réellement passé depuis le dernier récap ou depuis le début de la conversation.

À faire :

1. Crée le dossier `.claude/commands/` s'il n'existe pas déjà.
2. Crée ce fichier : `.claude/commands/session-recap.md`
3. Mets exactement le contenu suivant dans ce fichier :
...
* Ne renomme pas de commandes existantes.
* N'ajoute pas de dépendances.
* Ne change pas la configuration du projet.
* Garde le fichier de commande simple et lisible.

À la fin, indique simplement :

* le fichier créé ;
* comment utiliser la commande dans Claude Code, par exemple : `/session-recap`.
